...
namespace OfficeOpenXml.Drawing
{
    ...
    public class ExcelDrawings : IEnumerable<ExcelDrawing>, IDisposable
    {
        ...
        /// <returns></returns>
        public ExcelPicture AddPicture(string Name, SKBitmap bitmap)
        {
            return AddPicture(Name, bitmap, null);
        }
        /// <summary>
        /// Add a picure to the worksheet
        /// </summary>
        ...
        public ExcelPicture AddPicture(string Name, SKBitmap bitmap, Uri Hyperlink)
        {
            if (bitmap != null)
            {
                if (_drawingNames.ContainsKey(Name))
                ...
                ExcelPicture pic = new ExcelPicture(this, drawNode, SKImage.FromBitmap(bitmap), Hyperlink);  // Assuming ExcelPicture is adjusted to handle SKBitmap
                pic.Name = Name;
                _drawings.Add(pic);
                _drawingNames.Add(Name, _drawings.Count - 1);
                return pic;
            }
            throw (new Exception("AddPicture: Bitmap can't be null"));
        }
